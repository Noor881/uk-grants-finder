...
export default function TrainingDetail() {
  ...
  const pageTitle = item ? `${item.title} — UK Free Training & Courses` : 'UK Training Programme Details'
  const pageDesc = item
    ? `${item.title}: who can apply, course details, and how to enrol. ${item.description?.slice(0, 100) || ''}`.slice(0, 160)
    : 'UK government-funded training programme details and how to apply.'
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: item?.title || 'Programme', item: canonicalUrl },
    ]
  }
